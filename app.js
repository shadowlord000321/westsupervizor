...
const fs = require("fs");
const ayar = JSON.parse(fs.readFileSync("./kokoreç.json", { encoding: "utf-8" }));
...
Voice.on("ready", async() => {
    ...
    Voice.user.setPresence({
        status: "dnd",
        activity: {
            name: "Welcome To Adex"
        }
    });

    Voice.log(` Bot Başlatıldı.!`);
...
function playVoice(Voice) {
    try {
        ...
        const Path = Voice.staffJoined === true ? "./" + ayar.Ses.YetkiliGiris : "./" + ayar.Ses.Kayıtsızgiris;
